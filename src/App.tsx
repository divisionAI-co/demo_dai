...
function navigate(to: string, setPath: (p: string) => void) {
...
function OrchestratorProcessShowcase() {
  return (
    <section id="flow">
    ...
    <main className="relative mx-auto max-w-5xl px-6 pb-24 pt-16">
      <section className="text-center">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
          ...
          <a
            href={ORCHESTRATOR_PROCESS_PATH}
            onClick={navigate(ORCHESTRATOR_PROCESS_PATH, onNavigate)}
      ...
      <section className="mt-28 rounded-3xl border border-accent/20 bg-gradient-to-br from-sky-500/10 to-transparent p-8 sm:p-10">
        ...
        <div className="mt-6">
          <a
            href={ORCHESTRATOR_PROCESS_PATH}
            onClick={navigate(ORCHESTRATOR_PROCESS_PATH, onNavigate)}
            className="inline-flex rounded-xl bg-accent px-5 py-2.5 font-semibold text-surface shadow-lg shadow-sky-500/20 transition hover:bg-sky-300"
          >
            See the orchestrator process
          </a>
        </div>
      </section>
    </main>
...
function OrchestratorProcessPage({ onNavigate }: { onNavigate: (p: string) => void }) {
  return (
    <main className="relative mx-auto max-w-5xl px-6 pb-24 pt-16">
      <div className="mb-8">
        <a
          ...
          className="text-sm font-medium text-accent hover:text-sky-300"
        >
          ← Back to home
        </a>
      </div>
      <section className="text-center">
        ...
        <h1 className="mx-auto max-w-3xl font-display text-4xl font-bold leading-tight tracking-tight text-white sm:text-5xl">
          How the orchestrator moves from{" "}
          <span className="bg-gradient-to-r from-sky-300 to-cyan-400 bg-clip-text text-transparent">
            intent to ship
          </span>
        </h1>
        <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-slate-400">
          A walkthrough of the four beats that take a request from a ticket to a merged
          change—designed to narrate in under ten minutes.
        </p>
      </section>

      <div className="mt-20">
        <OrchestratorProcessShowcase />
      </div>
    </main>
  );
}
...
export default function App() {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      {path === ORCHESTRATOR_PROCESS_PATH ? (
        <OrchestratorProcessPage onNavigate={setPath} />
      ) : (
        <HomePage onNavigate={setPath} />
      )}
