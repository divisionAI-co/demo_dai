...
type Route = "home" | "process";

function routeFromPath(path: string): Route {
  return path === "/process" ? "process" : "home";
}

export default function App() {
  const initialPath =
    typeof window !== "undefined" ? window.location.pathname : "/";
  const [route, setRoute] = useState<Route>(routeFromPath(initialPath));
  ...
  const navigate = useCallback((path: "/" | "/process") => {
  ...
  return (
    <div className="relative overflow-hidden">
        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          ...
          <nav className="flex items-center gap-2">
            <a
              href="/"
              onClick={handleNav("/")}
              className={navLinkClass(route === "home")}
            >
              Home
            </a>
            <a
              ...
              className={navLinkClass(route === "process")}
            >
              Process
            </a>
            <span className="ml-2 hidden rounded-full border border-surface-border bg-surface-raised/50 px-3 py-1 text-xs font-medium text-slate-400 sm:inline-flex">
              Customer demo
            </span>
          </nav>
        </div>
      ...
      {route === "process" ? (
        <ProcessPage onNavigateHome={() => navigate("/")} />
      ) : (
